...
//  Load recent messages(maximum 20) of the room, whenever user opens the chat window of the room 
...
$('#message-list-top .simplebar-content-wrapper').scroll(function() {
 ...
 // Check if user has entered the room/group name
 ...
 // Check if room is already created or not
 async function checkRoomAvailability() {
     if(roomNameText.val().length !== 0) {
        var roomname = replaceWhiteSpaces(roomNameText.val());
        var alreadyJoined = false;

        //  Check if user is already a member of the room or not
        ...
        //  Check if room name is available or not
        var roomExists = true;
 ...
 // Join room or create a room 
async function joinRoom(roomName, roomExists) {

    //  Add the roomname to the user's room/group list
    ...
    // Send notification as a message to the room/group
...
//  open chat window of the room clicked
function openChat(roomID) {
    ...
    if(document.querySelector('.chat-block').style.visibility == "hidden") {
        ...
        //  detach our listener from the current room
        unsubscribe();
        document.querySelector('.messages').innerHTML = "";
        document.querySelector('.chat-name').innerHTML = `${currentRoomID}` ;
        //  Check if previous room was deleted or not, if not then change its background
...
//  show the list of rooms joined by the user on frontend
...
function leaveRoom() {
    ...
    //  Open room list for mobile users
    if(window.innerWidth < 551) {
        openRoomList();
    }
    var dateTime = getDateTime();
    ...
    //  reomve the roomname from the users joined room list
